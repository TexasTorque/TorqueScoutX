...
export const averagePoints = (data, analysisGroup) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        teamAverageObject[analysisGroup.name.replace(/\s/g, '').replaceAll('.', '')] = teamAverage.toFixed(2).toString();
...
export const booleanPercentage = (data, analysisGroup) => {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        teamAverageObject[analysisGroup.name.replace(/\s/g, '').replaceAll('.', '')] = (booleanPercentage.toFixed(2) * 100).toString() + '%';
