...
const Averages = () => {
    ...
    const populateColumns = (data) => {
        ...
        data[0][Object.keys(data[0])[0]].reports[0].fields.forEach((key) => {
            if ((typeof key.value) === "string") {
                clmTemp = [...clmTemp, makeColumn(key.name + " Points", key.name.replace(/\s/g, '').replaceAll('.', ''), true)]
            } else {
    ...
    useEffect(() => {
        ...
        let temp = allData.filter((row) => {
            return searchQuery === "" ? true : (row.Team.substring(0, searchQuery.length) === searchQuery);
        });
    ...
    const processData = (data) => {
        ...
        console.log(customProcessData(data).map((e, i) => {
            return {...e, ...allData[i]}
        }));
            ...
            <div className="home">
                <div className="mt-4" style={{ width: "200%" }}>
                    <Group name="Averages">
                        ...
                        style={{ width: "8em", height: "2em", marginTop: "1.6em", marginLeft: "1em" }}
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        />
                        <Button onClick={() => deleteTeam()} style={{marginLeft: "1em", marginRight: "1em", height: "2.1em", marginTop: "1.6em"}} name="Delete">Delete</Button>
                        <Button onClick={() => resetDeletes()} style={{height: "2.1em", marginTop: "1.6em"}} name="Reset All">Reset All</Button>
                        ...
                        <div className="table-container">
                            <Table json={allDataTable} defaultSortField="Points" columns={columns} />
                        </div>
                    </Group>
                </div>
            </div> : <Null />
